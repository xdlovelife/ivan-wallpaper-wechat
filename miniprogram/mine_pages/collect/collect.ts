...
Page({
  ...
  fetchResults: function(isRefresh:boolean, uid:string) {
    ...
    wx.request({
      url: `${app.globalData.baseURL}/manager/v2/wallpaper/wechat/collect/for-date`,
      method: 'POST',
      data: {
        page: page,
        ...
        if (res.statusCode === 200 && res.data.code === 0) {
          ...
          if (newItems && newItems.length > 0) {
            ...
            newItems.forEach(item => {
              // 判断是否有 serviceAccountActionVOList 数据
              if (item.serviceAccountActionVOList && item.serviceAccountActionVOList.length > 0) {
                  // 调用方法生成完整文件路径
                  const updatedItems = app.generateFullFilePaths(item.serviceAccountActionVOList);
                  
                  // 替换原集合中的壁纸数据
                  item.serviceAccountActionVOList = updatedItems;
              } else {
                  console.log('No serviceAccountActionVOList found for item', item);
              }
            });
